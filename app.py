...
st.set_page_config(page_title="Boîte à Outils Projet v8 — Toujours avancer", layout="wide")
st.title("🚀 Copilote Projet — Toujours avancer")

# -------------------------------
# Étape 1 : Où j'en suis
# -------------------------------
st.subheader("1️⃣ Où en est ton projet ?")
st.write("Déplace la fusée sur la trajectoire pour indiquer ton niveau actuel de maturité.")
...
objectif = st.selectbox("Sélectionne ton objectif :", options_objectif)
...
# Mapping fictif pour next moves
next_moves_map = {
...
# Générer next moves avec fallback action
next_moves = next_moves_map.get((closest_point["x"], besoin))
if not next_moves:
    next_moves = [f"Aucun outil direct disponible pour {besoin} ici → commence par préparer ton projet et explorer des alternatives concrètes"]
...
st.caption("Prototype UX v8 amélioré — jamais bloqué, next moves toujours visibles, expérience user-centric, progression ludique.")
